...
class Plugin(ShortcodePlugin):
    ...
    def handler(self, signal="info", title="", site=None, data=None, lang=None, post=None):
        ...
        if signal.lower() not in ["warning", "danger", "info", "success", "primary", "secondary", "light"]:
            signal = "info"
        ...
        if title:
            ...
            output = '<div class="alert alert-dismissible alert-{0}"><h4 class="alert-heading">{2}</h4> <p class="mb-0"> {1} </p> </div>'.format(signal, data, title)
        else:
            output = '<div class="alert alert-dismissible alert-{0}"> <p class="mb-0"> {1} </p> </div>'.format(signal, data)
